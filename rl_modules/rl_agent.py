...
class RLAgent:
    def __init__(self, args, compute_rew, goal_sampler, reward_func):
        ...
        if args.algo == 'language':
            ...
            self.language = False
        self.her_module = her_sampler(self.args, reward_func_true=compute_rew, reward_func_pred=reward_func)
    ...
    def train(self, reward_func=None):
        # train the network
        self.total_iter += 1
        self._update_network(reward_func=reward_func)
    ...
    # update the network
    def _update_network(self, reward_func=None):

        # sample from buffer, this is done with LP is multi-head is true
        transitions = self.buffer.sample(self.args.batch_size, reward_func=reward_func)
