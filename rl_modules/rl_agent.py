...
class RLAgent:
    ...
    def get_action_proba(self, action, obs, ag, g, mask, no_noise, language_goal=None):
        # apply mask
        if mask is not None:
            if self.args.mask_application == 'hindsight':
                g = g * (1 - mask) + ag * mask
        ...
        with torch.no_grad():
            ...
            if self.architecture == 'gnn':
                obs_tensor = torch.tensor(obs_norm, dtype=torch.float32).unsqueeze(0)
                action_proba = self.model.get_action_proba(action, obs_tensor, ag_norm, g_norm, no_noise=no_noise)
