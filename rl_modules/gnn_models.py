...
class GnnActor(nn.Module):
    ...
    def get_action_proba(self, action, obs, edge_features):
        mean, log_std = self.forward(obs, edge_features)
        std = log_std.exp()
        normal = Normal(mean, std)
        action_inverse_tanh = np.arctanh(action)
        action_proba = [x if x<0.5 else 1-x for x in normal.cdf(torch.tensor(action_inverse_tanh))[0]]
        #print(np.mean(action_proba))
        #import pdb;pdb.set_trace()
        return np.mean(action_proba)
...
class GnnSemantic:
    ...
    def get_action_proba(self, action, obs, ag, g, no_noise=False):
        edge_features = self.critic.message_passing(obs, ag, g)
        action_proba = self.actor.get_action_proba(action, obs, edge_features)
        return action_proba
