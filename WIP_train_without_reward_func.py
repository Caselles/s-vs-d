...
def launch(args):
    ...
    # Initialize reward function module
    reward_func = RewardFunction(args)

    # Initialize RL Agent
    if args.agent == "SAC":
        policy = RLAgent(args, env.compute_reward, goal_sampler, reward_func)
    else:
    ...
    for epoch in range(args.n_epochs):
        ...
        for _ in range(args.n_cycles):
            ...
            # Storing reward samples
            # with demos this will be a step, for now I'm just experimenting with a fake reward dataset
            '''t_i = time.time()
            reward_func.store(episodes) # should add the teacher feedback and demos
            time_dict['store_reward'] += time.time() - t_i'''

            # Policy updates
            t_i = time.time()
            for _ in range(args.n_batches):
                policy.train(reward_func=reward_func) #here we add the reward func for convenience for HER computation and
            time_dict['policy_train'] += time.time() - t_i
            episode_count += args.num_rollouts_per_mpi * args.num_workers

            # Reward function updates
            t_i = time.time()
            for _ in range(args.n_batches_reward):
                reward_func.train()
            time_dict['reward_func_train'] += time.time() - t_i
        ...
        if args.evaluations:
            ...
            if rank == 0:
                ...
                if epoch % args.save_freq == 0:
                    ...
                    reward_func.save(model_path, epoch)
