...
class Learner():
	...
	def __init__(self, env, pragmatic):
...
def test_learner_after_training(learner, teacher):
	...
	for g in range(3):
		...
		for t in range(nb_tests_per_goal):
			...
			success_goal_reachability = learner.policies[0].success_condition(predicted_goal_desired, goal_achieved)
	...
	for seed in range(150):
		...
		for pragmatic in [False, True]:
			...
			for pedagogical in [False, True]:
				...
				if pedagogical:
					save_path = 'results/learner/pedagogical/'
				else:
					save_path = 'results/learner/optimal/'
