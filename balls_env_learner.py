...
class PolicyLearner():
	...
	def update_policy(self, action, demo_goal_desired, goal_desired, predicted_goal_desired, goal_achieved, pragmatism=False):
		...
		if success:
			if pragmatism:
				self.action_proba[action[0]] += 0.02
				self.action_proba_knowing_s1[action[0]][action[1]] += 0.02
			else:
...
class Learner():
	...
	def update_policy(self, action, demo_goal_desired, goal_desired, predicted_goal_desired, goal_achieved, pragmatism=False):
		...
		self.policies[predicted_goal_desired[-1]].update_policy(action, demo_goal_desired, goal_desired, predicted_goal_desired, goal_achieved, pragmatism)
			...
			for pedagogical in [False, True]:
				...
				for i in range(2000):
					...
					if learner.pragmatic:
						predicted_goal_trajectory = learner.predict_goal_from_demo(action)
						prediction_goal_trajectory_success = predicted_goal_desired == predicted_goal_trajectory
						learner.update_policy(action, demo_goal_desired, goal_desired, predicted_goal_desired, goal_achieved, pragmatism=prediction_goal_trajectory_success)
					else:
