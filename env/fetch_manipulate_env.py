...
class FetchManipulateEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if self.reward_type == 'sparse':
            ...
            active_predicates_goal = np.where(goal == 1)
            reward = (achieved_goal[active_predicates_goal] == goal[active_predicates_goal]).all().astype(np.float32)
        else:
            reward = 0.
    ...
    def _is_success(self, achieved_goal, desired_goal):
        ...
        active_predicates_goal = np.where(desired_goal == 1)
        return (achieved_goal[active_predicates_goal] == desired_goal[active_predicates_goal]).all()
    ...
    def reset_goal_illustrative_example(self, goal, biased_init=False):
        """
        This function resets the environment and target the goal given as input
        Args:
            goal: The semantic configuration to target
        ...
        pos_stack = self.initial_gripper_xpos[:2] -0.2 #+ self.np_random.uniform(-self.obj_range, self.obj_range, size=2)
        pos_two_cubes = [pos_stack+0.05, pos_stack]
        for i, obj_name in enumerate(self.object_names):
            object_qpos = self.sim.data.get_joint_qpos('{}:joint'.format(obj_name))
            assert object_qpos.shape == (7,)
            if i in stack:
                object_qpos[2] = z_stack[stack.index(i)]
                object_qpos[:2] = pos_two_cubes[i].copy()
                ...
                while counter < 100:
                    ...
                    if np.linalg.norm(object_xpos - pos_stack) > (np.sqrt(2) * 0.1):
                        break
                object_qpos[:2] = object_xpos.copy()
            self.sim.data.set_joint_qpos('{}:joint'.format(obj_name), object_qpos)
